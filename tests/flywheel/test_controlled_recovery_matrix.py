...
def _accepted(root: Path, round_id: str, episode: str, category: str) -> dict[str, str]:
    ...
    manifest = {
        ...
        for path in (reset, annotations, continuation, raw / "episode.json")
    }
...
def test_builder_carries_the_exact_authenticated_continuation_state_into_portable_rows(tmp_path: Path) -> None:
    ...
    selected[0]["continuation_start"]["snapshot_sha256"] = _write(continuation, snapshot)
    selected[0]["continuation_start"]["snapshot_robot_position"] = selected[0]["continuation_start"]["state"]
    ...
    manifest["snapshots/continuations/000016.json"] = {"sha256": hashlib.sha256(continuation.read_bytes()).hexdigest(), "size": continuation.stat().st_size}
...
def test_builder_binds_the_authenticated_source_reset_seed_not_the_perturbation_seed(tmp_path: Path) -> None:
    """The source simulator seed must survive audit hydration for replay fidelity."""
    ...
    rows = json.loads(Path(result["materialization_path"]).read_text(encoding="utf-8"))["rows"]
    assert {row["source_seed"] for row in rows} == {50110}
    assert {row["perturbation_seed"] for row in rows} == set(range(71_000, 71_008))
...
def test_portable_matrix_is_byte_identical_after_accepted_root_relocation(tmp_path: Path) -> None:
    ...
    def build_fixture(base: Path) -> tuple[Path, Path]:
        ...
        selected = [_accepted(accepted, "round-1", "long", "pant_long"), _accepted(accepted, "round-1", "tl", "top_long"), _accepted(accepted, "round-1", "ts", "top_short")]
        audit = _audit(selected)
        audit_path = base / "recovery-audit-v3.json"
        _write(audit_path, audit)
...
def test_legacy_v2_audit_with_116_successes_requires_fresh_snapshot_sources(tmp_path: Path) -> None:
    from scripts.plan_continuation_snapshot_backfill import plan_continuation_snapshot_backfill

    audit = tmp_path / "recovery-audit-v2.json"
    _write(audit, {
        "schema_version": 2,
        "kind": "lehome_successful_recovery_audit",
        "admitted_episodes": [{"source_episode_id": f"success-{index:03d}"} for index in range(116)],
        "selected_recoveries": [{"source_episode_id": "success-000"}],
    })
    plan = plan_continuation_snapshot_backfill(audit_path=audit)
    assert plan["existing_accepted_success_count"] == 116
