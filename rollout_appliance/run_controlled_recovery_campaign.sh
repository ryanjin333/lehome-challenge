...
if not isinstance(local, dict) or local.get("schema_version") != 2 or local.get("kind") != "controlled_success_recovery_materialization_v2" or local.get("matrix_sha256") != expected or local.get("target_accepted") != 8 or local.get("category_acceptance_caps") != caps or not isinstance(local.get("rows"), list) or len(local["rows"]) != attempts: raise SystemExit("controlled recovery materialization does not bind matrix")
seen, attempt_ids, trial_ids, seeds, perturbations = set(), set(), set(), set(), set()
for row, hydrated in zip(rows, local["rows"], strict=True):
    if not isinstance(row, dict) or row.get("recovery_kind") != "controlled_success_recovery_snapshot_v2" or row.get("category_acceptance_cap") != caps.get(row.get("category")) or row.get("strategy") != "canonical": raise SystemExit("controlled recovery row is invalid")
