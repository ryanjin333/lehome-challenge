...
if not isinstance(local, dict) or local.get("schema_version") != 1 or local.get("kind") != "controlled_success_recovery_materialization_v1" or local.get("matrix_sha256") != expected or local.get("target_accepted") != 8 or local.get("category_acceptance_caps") != caps or not isinstance(local.get("rows"), list) or len(local["rows"]) != attempts: raise SystemExit("controlled recovery materialization does not bind matrix")
seen, attempt_ids, trial_ids, seeds, perturbations = set(), set(), set(), set(), set()
for row, hydrated in zip(rows, local["rows"], strict=True):
    if not isinstance(row, dict) or row.get("recovery_kind") != "controlled_success_recovery_v1" or row.get("category_acceptance_cap") != caps.get(row.get("category")) or row.get("strategy") != "canonical": raise SystemExit("controlled recovery row is invalid")
    ...
    if not isinstance(hydrated, dict) or hydrated.get("controlled_matrix_sha256") != expected or any(not isinstance(hydrated.get(key), str) or not hydrated[key].startswith("/") or Path(hydrated[key]).is_symlink() or not Path(hydrated[key]).is_file() for key in ("source_reset", "source_annotations")): raise SystemExit("controlled recovery materialization paths are unsafe")
    if {key: value for key, value in hydrated.items() if key not in {"source_reset", "source_annotations", "controlled_matrix_sha256"}} != row: raise SystemExit("controlled recovery materialization differs from matrix")
    if not isinstance(row.get("prefix_stop"), int) or not isinstance(row.get("source_first_success_step"), int) or not 0 < row["prefix_stop"] < row["source_first_success_step"]: raise SystemExit("controlled recovery prefix is invalid")
    for key in ("source_reset_sha256", "source_annotations_sha256", "action_prefix_sha256", "source_state_fingerprint", "perturbation_fingerprint", "source_state_perturbation_fingerprint", "source_episode_digest"):
    ...
    if hashlib.sha256(Path(hydrated["source_reset"]).read_bytes()).hexdigest() != row["source_reset_sha256"] or hashlib.sha256(Path(hydrated["source_annotations"]).read_bytes()).hexdigest() != row["source_annotations_sha256"]: raise SystemExit("controlled recovery materialization source hashes are invalid")
