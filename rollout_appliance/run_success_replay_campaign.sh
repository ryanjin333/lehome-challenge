...
exec env \
  ...
  LEHOME_RUN_ID="${LEHOME_RUN_ID:-success-replay-run}" \
  LEHOME_HOST_CODE_ROOT="${LEHOME_HOST_CODE_ROOT:-}" \
  LEHOME_ONE_VM_ORCHESTRATOR="${LEHOME_ONE_VM_ORCHESTRATOR:-0}" \
  LEHOME_PAID_COLLECTION="${LEHOME_PAID_COLLECTION:-0}" \
  LEHOME_ROLLOUT_IMAGE="${LEHOME_ROLLOUT_IMAGE:-}" \
  LEHOME_TRAINER_IMAGE="${LEHOME_TRAINER_IMAGE:-}" \
  LEHOME_ENABLE_HF_UPLOAD="${LEHOME_ENABLE_HF_UPLOAD:-}" \
  LEHOME_SKIP_ROUND_SEAL="${LEHOME_SKIP_ROUND_SEAL:-}" \
  bash "${BASE_CAMPAIGN}"
