...
SOURCE_FRESH_GARMENT_WAVES=0
if [ "${SNAPSHOT_SOURCE_SIMULATOR_DEVICE}" = "cpu" ]; then
  SOURCE_FRESH_GARMENT_WAVES=1
fi
...
env LEHOME_WORKSPACE="${WORKSPACE}" LEHOME_CAMPAIGN_ROOT="${ROOT}" \
  ...
  LEHOME_FRESH_GARMENT_WAVES="${SOURCE_FRESH_GARMENT_WAVES}" \
