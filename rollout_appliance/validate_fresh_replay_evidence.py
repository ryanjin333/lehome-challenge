#!/usr/bin/env python3
"""Fail-closed authentication for the exact 200-row visual-only replay mode.

Both replay shell entrypoints execute this file before they admit the raised
acceptance cap.  Keeping the complete source-evidence walk here prevents a
wrapper/base-campaign validation split from becoming a bypass.
"""
...
def _evidence(raw: str, *, label: str) -> dict[str, tuple[str, Path]]:
    ...
    for item in values:
        ...
        if _sha256(path) != digest or str(path) in found:
...
def validate_exact_fresh_visual_only(
    *, matrix_path: str | Path, max_attempts: int, source_reports_json: str, source_matrices_json: str,
    ...
    reports = _evidence(source_reports_json, label="source report")
    raw_matrices = _evidence(source_matrices_json, label="source matrix")
    matrices: dict[str, tuple[str, Mapping[str, Mapping[str, object]]]] = {}
    matrix_attempts: set[str] = set()
    for path_text, (digest, path) in raw_matrices.items():
        source_rows = _load_json(path, label="fresh source matrix", object_only=False)
        if not isinstance(source_rows, list) or not source_rows or digest in matrices:
            raise ValueError("fresh source matrix is malformed")
        by_attempt: dict[str, Mapping[str, object]] = {}
        for source_row in source_rows:
            if not isinstance(source_row, Mapping) or not isinstance(source_row.get("attempt_id"), str):
                raise ValueError("fresh source matrix row is malformed")
            attempt = str(source_row["attempt_id"])
            if (
                attempt in matrix_attempts or source_row.get("trial_id") != attempt
                or source_row.get("category") not in CATEGORIES
                or not isinstance(source_row.get("garment_name"), str) or not source_row["garment_name"]
                or source_row.get("release_stage") != "seen" or source_row.get("strategy") != "canonical"
                or source_row.get("campaign_kind") != FRESH_SOURCE_CAMPAIGN_KIND
                or source_row.get("logical_stage") != FRESH_SOURCE_LOGICAL_STAGE
                or not isinstance(source_row.get("campaign_round_id"), str)
                or not isinstance(source_row.get("campaign_run_id"), str)
            ):
                raise ValueError("fresh source matrix identity is invalid")
            matrix_attempts.add(attempt)
            by_attempt[attempt] = source_row
        matrices[digest] = (path_text, by_attempt)

    report_trials: dict[str, tuple[Mapping[str, object], Mapping[str, object], str, str, Mapping[str, Mapping[str, object]]]] = {}
    for report_path, (report_digest, path) in reports.items():
        report = _load_json(path, label="fresh source report")
        if not isinstance(report, Mapping):
            raise ValueError("fresh source report is malformed")
        matrix_path_text, matrix_digest, matrix_rows, trials = _validate_report(
            report, matrices=matrices,
        )
        for attempt, trial in trials.items():
            if attempt in report_trials:
                raise ValueError("every fresh source must appear in exactly one report and source matrix")
            report_trials[attempt] = (trial, report, report_digest, matrix_path_text, matrix_rows)
    ...
    for row in rows:
        ...
        root = _absolute_directory(row.get("source_episode_root"), label="fresh source artifact root")
        episode = _regular_absolute(row.get("source_episode_path"), label="fresh source episode")
        snapshot = _regular_absolute(row.get("restore_snapshot"), label="fresh source continuation snapshot")
        receipt = _regular_absolute(row.get("source_receipt_path"), label="fresh source receipt")
        reset = root / "raw" / parent / "snapshots" / "reset.json"
        annotations = root / "raw" / parent / "annotations.jsonl"
        expected_episode = root / "raw" / parent / "episode.json"
        expected_snapshot = root / "raw" / parent / "snapshots" / "continuations" / "000016.json"
        if episode != expected_episode or snapshot != expected_snapshot:
            raise ValueError("fresh source episode path is not canonical")
        ...
        if (
            _episode_artifact_sha256(root) != row.get("source_episode_sha256")
            or checked["reset"] != row.get("source_reset_sha256")
            or checked["annotations"] != row.get("source_annotations_sha256")
            or checked["h16"] != row.get("source_continuation_snapshot_sha256")
            or _sha256(snapshot) != row.get("restore_snapshot_sha256")
        ...
        _canonical_episode(episode_json, attempt_id=parent, category=str(category), garment=str(row.get("garment")), round_id=str(round_id), run_id=str(run_id))
        _canonical_snapshot(reset_json, garment=str(row.get("garment")), continuation=False)
        continuation = _canonical_snapshot(snapshot_json, garment=str(row.get("garment")), continuation=True)
        if _state_fingerprint(category=str(category), garment=str(row.get("garment")), continuation=continuation) != row.get("source_state_fingerprint"):
            raise ValueError("fresh source continuation state fingerprint is invalid")
        receipt_json = _load_json(receipt, label="fresh source receipt")
        if (
            not isinstance(receipt_json, Mapping) or _sha256(receipt) != row.get("source_receipt_sha256")
            or receipt_json.get("readback_verified") is not True or receipt_json.get("attempt_id") != parent
            or receipt_json.get("round_id") != round_id or receipt_json.get("run_id") != run_id
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        validate_exact_fresh_visual_only(
            matrix_path=args.matrix, max_attempts=args.max_attempts,
