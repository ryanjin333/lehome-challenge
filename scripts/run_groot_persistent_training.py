...
def _final_runtime_gpu_datacenter_offer(
    *, runner: Runner, excluded_machine_id: int | None = None,
    ...
    if not isinstance(rows, list):
        raise ValueError("final on-demand GPU offer search is invalid")
    eligible = [
        ...
        and row.get("rentable") is True and row.get("rented") is False
        and _runtime_gpu_datacenter_row(row)
        and type(row.get("dph_total")) in (int, float)
        and math.isfinite(float(row["dph_total"])) and float(row["dph_total"]) > 0
    ]
    if not eligible or len({row["id"] for row in eligible}) != len(eligible):
        raise ValueError("final on-demand GPU offer is unavailable or ambiguous")
    selected = min(eligible, key=lambda row: (float(row["dph_total"]), int(row["id"])))
    return _project(selected, (
        "id", "machine_id", "gpu_name", "gpu_ram", "num_gpus", "dph_total",
        "driver_version", "datacenter", "is_bid", "rentable", "rented",
    ))


_RUNTIME_GPU_ATTEMPT_REQUIRED_FIELDS = frozenset({
    "id", "machine_id", "gpu_name", "gpu_ram", "num_gpus", "dph_total",
    "is_bid", "rentable", "rented",
})
_RUNTIME_GPU_ATTEMPT_FIELDS = _RUNTIME_GPU_ATTEMPT_REQUIRED_FIELDS | {"driver_version", "datacenter"}


def _runtime_gpu_attempted_offer(value: Mapping[str, object]) -> dict[str, object]:
    """Freeze the exact ephemeral row that is permitted to reach create."""
    offer = _project(value, tuple(_RUNTIME_GPU_ATTEMPT_FIELDS))
    if (
        not _RUNTIME_GPU_ATTEMPT_REQUIRED_FIELDS.issubset(offer)
        or set(offer) - _RUNTIME_GPU_ATTEMPT_FIELDS
        or not _positive_int(offer.get("id")) or not _positive_int(offer.get("machine_id"))
        or type(offer.get("gpu_ram")) not in (int, float) or not _offer_gpu(offer)
        or offer.get("num_gpus") != 1 or offer.get("is_bid") is not False
        or offer.get("rentable") is not True or offer.get("rented") is not False
        or not _runtime_gpu_datacenter_row(offer)
        or type(offer.get("dph_total")) not in (int, float)
        or not math.isfinite(float(offer["dph_total"])) or float(offer["dph_total"]) <= 0
    ):
        raise ValueError("final on-demand GPU offer is invalid")
    return offer
...
def rent(*, evidence: Mapping[str, object], runner: Runner, max_readiness_polls: int = 12, sleep: Callable[[float], None] = _bounded_sleep, require_capability: bool = True, abort_request: Mapping[str, object] | None = None, direct_gpu_on_demand: bool = False) -> dict[str, object]:
    ...
    try:
        ...
        if direct_gpu_on_demand:
            ...
            _runtime_gpu_attempted_offer(offer)
        else:
            _require_account_cap(
...
def _ssh_prefix(instance: Mapping[str, object]) -> tuple[str, ...]:
    ...
    return ("ssh", "-o", "IdentitiesOnly=yes", "-i", str(identity), "-o", "ClearAllForwardings=yes", "-o", "BatchMode=yes", "-o", "ServerAliveInterval=15", "-o", "ServerAliveCountMax=4", "-o", "StrictHostKeyChecking=accept-new", "-p", str(port), "root@" + host)
...
def _scp_prefix(instance: Mapping[str, object], *, recursive: bool = False) -> tuple[str, ...]:
    ...
    return (
        ...
        "-i", str(identity), "-o", "ServerAliveInterval=15", "-o", "ServerAliveCountMax=4",
...
def _runtime_gpu_rent_preflight(
    ...
    if recovery_safe and search_mode not in {"interruptible", "on_demand"}:
        raise ValueError("runtime GPU recovery original offer mode is invalid")
    if not recovery_safe and search_mode != "on_demand":
...
def _runtime_gpu_rent_claim(
    *, path: Path, request: Mapping[str, object], identity: Mapping[str, object],
    attempted_offer: Mapping[str, object] | None = None,
) -> dict[str, object]:
    ...
    if request.get("search_mode") == "on_demand":
        attempt = _runtime_gpu_attempted_offer(
            offer if attempted_offer is None else attempted_offer,
        )
        claim |= {
            "attempted_offer": attempt,
            "attempted_offer_sha256": _hash(attempt),
        }
    try:
...
def _valid_blocked_runtime_gpu_claim(
    *, claim: Mapping[str, object], identity: Mapping[str, object], request: Mapping[str, object],
    ...
    if is_on_demand:
        fields |= {"attempted_offer", "attempted_offer_sha256"}
    attempt = claim.get("attempted_offer")
    try:
        attempt_is_authenticated = (
            isinstance(attempt, Mapping)
            and claim.get("attempted_offer_sha256") == _hash(dict(attempt))
            and _runtime_gpu_attempted_offer(attempt) == dict(attempt)
        )
    except ValueError:
        attempt_is_authenticated = False
    return (
        set(claim) == fields
...
def _runtime_gpu_claimed_attempted_offer(claim: Mapping[str, object]) -> dict[str, object]:
    """Load the exact row durably bound before an ambiguous direct create."""
    value = claim.get("attempted_offer")
    if not isinstance(value, Mapping) or claim.get("attempted_offer_sha256") != _hash(dict(value)):
        raise ValueError("runtime GPU claim attempted offer is not authenticated")
    return _runtime_gpu_attempted_offer(value)
...
def _runtime_gpu_recovery_request(
    ...
    original = _load_regular_json(blocked, "blocked runtime GPU rent request")
    original_offer = original.get("offer") if isinstance(original, Mapping) else None
    if original.get("search_mode") == "interruptible":
    ...
    elif original.get("search_mode") != "on_demand" or not isinstance(original_offer, Mapping):
        raise ValueError("runtime GPU recovery request must bind the authenticated on-demand machine")
...
def _runtime_gpu_recovery_offer_snapshot(
    *, runner: Runner, original_offer: Mapping[str, object], expected_machine_id: int,
    search_mode: str, timestamp_unix: int,
) -> tuple[str, dict[str, object]]:
    original_offer_id = original_offer.get("id")
    ...
    proof = _runtime_gpu_recovery_live_offer(
        runner=lambda _command: json.dumps(value), original_offer_id=int(original_offer_id),
...
def _runtime_gpu_recovery_reconciled_receipt_is_valid(
    ...
    search_mode: str = "interruptible", original_offer: Mapping[str, object] | None = None,
    blocked_rent_request: str | None = None,
) -> bool:
    ...
    if search_mode not in {"interruptible", "on_demand"}:
        return False
    is_on_demand = search_mode == "on_demand"
    if is_on_demand and (
        receipt.get("original_search_mode") != "on_demand"
        or receipt.get("blocked_rent_request") != blocked_rent_request
        or not isinstance(original_offer, Mapping)
    ):
        return False
    ...
    return (
        ...
        and (
            _runtime_gpu_recovery_on_demand_offer_proof_is_valid(receipt.get("start_offer_proof"), original_offer=original_offer, machine_id=machine_id)
            and _runtime_gpu_recovery_on_demand_offer_proof_is_valid(receipt.get("end_offer_proof"), original_offer=original_offer, machine_id=machine_id)
            if is_on_demand and isinstance(original_offer, Mapping) else
            _runtime_gpu_recovery_offer_proof_is_valid(receipt.get("start_offer_proof"), offer_id=original_offer_id, machine_id=machine_id)
...
def recover_runtime_gpu_rent(
    *, request: Mapping[str, object], runner: Runner,
    ...
    if not isinstance(original_offer, Mapping) or not _positive_int(original_offer.get("id")):
        raise ValueError("runtime GPU recovery original offer is invalid")
    original_search_mode = str(original.get("search_mode"))
    ...
    recovery_offer: Mapping[str, object] = original_offer
    if original_search_mode == "on_demand":
        recovery_offer = _runtime_gpu_claimed_attempted_offer(claim_value)
        if expected_machine_id != recovery_offer.get("machine_id"):
            raise ValueError("runtime GPU recovery request must bind the authenticated on-demand machine")
    ...
    if receipt == initial_receipt:
        mode, start_offer = _runtime_gpu_recovery_offer_snapshot(
            runner=runner, original_offer=recovery_offer, expected_machine_id=expected_machine_id,
            search_mode=str(original["search_mode"]),
        ...
        archived_current = {
            ...
            "reconciled_machine_id": expected_machine_id if mode == "present" else None, "original_offer_id": recovery_offer["id"],
        }
        final = {
            "schema_version": 1, "kind": "runtime_mixture_gpu_rent_recovery_receipt",
            "status": "reconciled", "released": False,
            "canonical_claim_path": str(claim_path), "canonical_claim_sha256": claim_sha256,
            "archive_claims": archives + [archived_current], "original_offer_id": recovery_offer["id"],
        ...
        if original_search_mode == "on_demand":
            final |= {"original_search_mode": "on_demand", "blocked_rent_request": str(blocked_path)}
    ...
    expected_archives = [row for row in archives if row["relative_filename"] != archive_path.name] + [{"relative_filename": archive_path.name, "byte_sha256": claim_sha256, "request_sha256": claim_value["request_sha256"], "offer_sha256": claim_value["offer_sha256"], "reconciled_machine_id": current_machine, "original_offer_id": recovery_offer["id"]}]
    if receipt.get("status") == "released":
        ...
        if (
            ...
            or not _runtime_gpu_recovery_reconciled_receipt_is_valid(
                ...
                original_offer_id=int(recovery_offer["id"]), machine_id=expected_machine_id,
                archives=expected_archives, search_mode=original_search_mode,
                original_offer=recovery_offer,
                blocked_rent_request=str(blocked_path) if original_search_mode == "on_demand" else None,
            )
        ):
    ...
    if not _runtime_gpu_recovery_reconciled_receipt_is_valid(receipt, claim_path=claim_path, claim_sha256=claim_sha256, original_offer_id=int(recovery_offer["id"]), machine_id=expected_machine_id, archives=expected_archives, search_mode=original_search_mode, original_offer=recovery_offer, blocked_rent_request=str(blocked_path) if original_search_mode == "on_demand" else None):
...
def _validate_runtime_gpu_recovery_for_new_rent(
    *, request: Mapping[str, object], identity: Mapping[str, object], claim_path: Path,
    ...
    original_search_mode = receipt.get("original_search_mode", "interruptible")
    if original_search_mode not in {"interruptible", "on_demand"}:
        raise ValueError("runtime GPU recovery receipt original mode is invalid")
    fields = {
        "schema_version", "kind", "status", "released", "canonical_claim_path",
        "canonical_claim_sha256", "archive_claims", "original_offer_id", "reconciled_machine_id",
        "observations", "start_offer_proof", "start_offer_proof_sha256", "end_offer_proof",
    ...
    if (
        ...
        or (original_search_mode == "interruptible" and mode == "absent" and receipt.get("blacklisted_machine_id") != RUNTIME_GPU_LEGACY_RECOVERY_BLACKLISTED_MACHINE_ID)
    ):
    ...
    original_offer: Mapping[str, object] | None = None
    blocked_rent_request: str | None = None
    if original_search_mode == "on_demand":
        blocked_rent_request = receipt.get("blocked_rent_request") if isinstance(receipt.get("blocked_rent_request"), str) else None
        blocked_path = Path(blocked_rent_request) if blocked_rent_request is not None else None
        if blocked_path is None or not blocked_path.is_absolute() or blocked_path.is_symlink() or not blocked_path.is_file():
            raise ValueError("runtime GPU recovery receipt original request is invalid")
        original = _load_regular_json(blocked_path, "blocked runtime GPU rent request")
        captured_offer = original.get("offer") if isinstance(original, Mapping) else None
        try:
            original_offer = _runtime_gpu_claimed_attempted_offer(archived_claim)
        except ValueError as error:
            raise ValueError("runtime GPU recovery receipt attempted offer is not authenticated") from error
        expected_machine = receipt.get("reconciled_machine_id") if mode == "present" else receipt.get("blacklisted_machine_id")
        if (
            original.get("search_mode") != "on_demand" or not isinstance(captured_offer, Mapping)
            or original_offer.get("id") != receipt.get("original_offer_id")
            or original_offer.get("machine_id") != expected_machine
            or archived_claim.get("request_sha256") != _hash({key: value for key, value in original.items() if key != "rent_claim_receipt"})
            or archived_claim.get("offer_sha256") != _hash(dict(captured_offer))
        ):
            raise ValueError("runtime GPU recovery receipt original request is not authenticated")
    ...
    if not _runtime_gpu_recovery_reconciled_receipt_is_valid(
        ...
        search_mode=str(original_search_mode), original_offer=original_offer,
        blocked_rent_request=blocked_rent_request,
    ):
...
def rent_runtime_gpu_warmup(*, evidence: Mapping[str, object], runner: Runner) -> dict[str, object]:
    ...
    attempted_evidence = dict(evidence) | {"offer": attempted_offer}
    rented: Mapping[str, object] | None = None
    try:
        rented = rent(
            evidence=attempted_evidence, runner=runner, require_capability=False,
...
def _runtime_hydration_job_dir(request_sha256: str) -> str:
    if re.fullmatch(r"[0-9a-f]{64}", request_sha256) is None:
        raise ValueError("runtime hydration job identity is invalid")
...
def _runtime_gpu_hydration_recovery_intent_path(
    *, instance: Mapping[str, object], request_sha256: str,
) -> Path:
    """Keep a rate-limit restart in the same controller-owned lease namespace."""
    base = _runtime_gpu_hydration_launch_intent_path(
        instance=instance, request_sha256=request_sha256,
    )
    return base.with_name(base.stem + "-rate-limit-recovery.json")


def _runtime_gpu_hydration_recovery_intent(
    *, instance: Mapping[str, object], request: Mapping[str, object], request_sha256: str,
    capability_sha256: str, partial_sources: Mapping[str, object],
) -> bool:
    """Admit one recovery controller; later callers may attach but never relaunch."""
    path = _runtime_gpu_hydration_recovery_intent_path(
        instance=instance, request_sha256=request_sha256,
    )
    immutable = {
        "schema_version": 1, "kind": "runtime_mixture_hydration_rate_limit_recovery_intent",
        "instance_id": instance["instance_id"],
        "provider_response_sha256": instance.get("provider_response_sha256"),
        "trainer_image": instance.get("trainer_image"), "image_digest": instance.get("image_digest"),
        "code_revision": request.get("code_revision"), "code_bundle_sha256": request.get("code_bundle_sha256"),
    ...
    expected = dict(immutable) | {"partial_sources": dict(partial_sources)}

    def attached() -> bool:
        for _ in range(20):
            if path.is_symlink():
                raise ValueError("runtime hydration recovery intent is tampered or incompatible")
            try:
                observed = dict(_load_regular_json(path, "runtime hydration recovery intent"))
            except ValueError:
                time.sleep(0.01)
                continue
            snapshot = observed.pop("partial_sources", None)
            if (
                observed != immutable
                or not isinstance(snapshot, Mapping)
                or _runtime_gpu_hydration_rate_limit_terminal(json.dumps(snapshot), request_sha256=request_sha256) is None
            ):
                raise ValueError("runtime hydration recovery intent is tampered or incompatible")
            return True
        raise ValueError("runtime hydration recovery intent is malformed or incomplete")
    ...
    try:
        ...
        return attached()
    if dict(_load_regular_json(path, "runtime hydration recovery intent")) != expected:
        raise RuntimeError("runtime hydration recovery intent readback mismatches")
    return False
...
def _runtime_gpu_hydration_rate_limit_terminal_command(request_sha256: str) -> str:
    ...
    job = _runtime_hydration_job_dir(request_sha256)
    return (
        "set -eu; # rate-limit-terminal\n"
        "job=" + shlex.quote(job) + "; sha=" + shlex.quote(request_sha256) + "; "
...
def _runtime_gpu_hydration_rate_limit_terminal(
    value: str, *, request_sha256: str,
) -> dict[str, object] | None:
    """Parse the sole terminal form eligible for a same-lease retry."""
    try:
    ...
    if (
        ...
        or set(terminal) != {"schema_version", "kind", "request_sha256", "exit_code", "organizer_files", "rollout_files"}
        or terminal.get("schema_version") != 1
        or terminal.get("kind") != "runtime_mixture_hydration_rate_limit_terminal"
        or terminal.get("request_sha256") != request_sha256
        or terminal.get("exit_code") != 1
        or any(type(terminal.get(key)) is not int or int(terminal[key]) < 0 for key in ("organizer_files", "rollout_files"))
    ):
        return None
    return dict(terminal)


def _runtime_gpu_hydration_recovery_job_start_command(request_sha256: str, *, initial_launch: bool) -> str:
    """Resume stable source targets once after an authenticated exhausted limit."""
    job = _runtime_hydration_job_dir(request_sha256)
    ...
    worker = (
        ...
        "printf '%s\\n' \"$$\" > \"$job/pid.tmp\"; mv \"$job/pid.tmp\" \"$job/pid\"; chmod 400 \"$job/pid\"; "
        "printf 'running %s\\n' \"$sha\" > \"$job/status.tmp\"; mv \"$job/status.tmp\" \"$job/status\"; chmod 400 \"$job/status\"; "
        "attempt=1; max_attempts=" + str(RUNTIME_GPU_HYDRATION_RECOVERY_RATE_LIMIT_ATTEMPTS) + "; while :; do if HF_TOKEN=\"$(cat /prepared/config/runtime.token)\" "
        "PYTHONPATH=/prepared/code/source/lehome:/prepared/code/trainer/src " + RUNTIME_GPU_PYTHON
        + " -m lehome_train.cli hydrate-runtime-mixture --request /prepared/config/runtime-hydrate.json "
        " >\"$job/receipt.tmp\" 2>\"$job/stderr.log\"; then "
        "test -s \"$job/receipt.tmp\"; mv \"$job/receipt.tmp\" \"$job/receipt.json\"; chmod 400 \"$job/receipt.json\" \"$job/stderr.log\"; "
        "printf 'success %s\\n' \"$sha\" > \"$job/terminal.tmp\"; mv \"$job/terminal.tmp\" \"$job/terminal\"; chmod 400 \"$job/terminal\"; break; "
        "else code=$?; if test \"$attempt\" -lt \"$max_attempts\" && grep -Fq 'Hub download rate limited after 1 attempts' \"$job/stderr.log\"; then "
        "rm -f \"$job/receipt.tmp\"; attempt=$((attempt + 1)); /usr/bin/sleep " + str(RUNTIME_GPU_HYDRATION_RECOVERY_RATE_LIMIT_DELAY_SECONDS) + "; continue; fi; "
        "rm -f \"$job/receipt.tmp\"; chmod 400 \"$job/stderr.log\"; printf 'failure %s %s\\n' \"$sha\" \"$code\" > \"$job/terminal.tmp\"; "
        "mv \"$job/terminal.tmp\" \"$job/terminal\"; chmod 400 \"$job/terminal\"; break; fi; done"
    )
    prefix = (
        "set -eu; job=" + shlex.quote(job) + "; recovery=" + shlex.quote(recovery) + "; sha=" + shlex.quote(request_sha256) + "; "
        "test -d \"$job\"; test ! -L \"$job\"; test -f \"$job/terminal\"; test ! -L \"$job/terminal\"; "
        "test -f \"$job/status\"; test ! -L \"$job/status\"; test \"$(cat \"$job/status\")\" = \"running $sha\"; "
        "test \"$(cat \"$job/terminal\")\" = \"failure $sha 1\"; test -f \"$job/stderr.log\"; test ! -L \"$job/stderr.log\"; "
        "grep -Fq 'Hub download rate limited after 1 attempts' \"$job/stderr.log\"; "
        "sources=/prepared/sources; test -d \"$sources\"; test ! -L \"$sources\"; "
        "for source in organizer rollout; do tree=\"$sources/$source\"; test -d \"$tree\"; test ! -L \"$tree\"; "
    ...
    if initial_launch:
        return (
            prefix + "if mkdir \"$recovery\" 2>/dev/null; then chmod 700 \"$recovery\"; printf '%s\\n' \"$sha\" > \"$recovery/request-sha256\"; chmod 400 \"$recovery/request-sha256\"; "
            "printf 'initializing %s\\n' \"$sha\" > \"$recovery/status\"; chmod 400 \"$recovery/status\"; "
            "/usr/bin/setsid /usr/bin/nohup /bin/sh -c " + shlex.quote(worker) + " sh \"$recovery\" \"$sha\" </dev/null >/dev/null 2>&1 & printf 'recovery-started\\n'; "
            "else test -d \"$recovery\"; test ! -L \"$recovery\"; printf 'recovery-attached\\n'; fi"
        )
    return (
        prefix + "test -d \"$recovery\"; test ! -L \"$recovery\"; test -f \"$recovery/request-sha256\"; test ! -L \"$recovery/request-sha256\"; "
        "test \"$(cat \"$recovery/request-sha256\")\" = \"$sha\"; test -f \"$recovery/status\"; test ! -L \"$recovery/status\"; printf 'recovery-attached\\n'"
    )


def _runtime_gpu_hydration_recovery_job_status_command(request_sha256: str) -> str:
    job = _runtime_hydration_job_dir(request_sha256) + "/recovery"
    return (
        ...
        "else test -f \"$job/pid\"; test ! -L \"$job/pid\"; pid=$(cat \"$job/pid\"); case \"$pid\" in *[!0-9]*|'') exit 9;; esac; if kill -0 \"$pid\" 2>/dev/null; then printf 'running\\n'; else printf 'disappeared\\n'; fi; fi; else exit 9; fi"
    )


def _runtime_gpu_hydration_recovery_job_receipt_command(request_sha256: str) -> str:
    job = _runtime_hydration_job_dir(request_sha256) + "/recovery"
    return "set -eu; job=" + shlex.quote(job) + "; test -f \"$job/receipt.json\"; test ! -L \"$job/receipt.json\"; cat \"$job/receipt.json\""


def _runtime_gpu_hydration_job_status_command(request_sha256: str) -> str:
    """Read only the authenticated job state; logs are intentionally never returned."""
    job = _runtime_hydration_job_dir(request_sha256)
    return (
        "set -eu; job=" + shlex.quote(job) + "; sha=" + shlex.quote(request_sha256) + "; "
        "test -d \"$job\"; test ! -L \"$job\"; test -f \"$job/request-sha256\"; test ! -L \"$job/request-sha256\"; "
...
def _runtime_gpu_hydration_job_receipt_command(request_sha256: str) -> str:
    job = _runtime_hydration_job_dir(request_sha256)
    return "set -eu; job=" + shlex.quote(job) + "; test -f \"$job/receipt.json\"; test ! -L \"$job/receipt.json\"; cat \"$job/receipt.json\""


def _runtime_hydration_ssh_prefix(instance: Mapping[str, object]) -> tuple[str, ...]:
    """A bounded reconnect attempt for detached hydration state only."""
    prefix = _ssh_prefix(instance)
    return (*prefix[:-1], "-o", "ConnectTimeout=15", prefix[-1])


def _runtime_hydration_transport_error(error: BaseException) -> bool:
    return isinstance(error, (OSError, TimeoutError, subprocess.TimeoutExpired)) or (
        isinstance(error, subprocess.CalledProcessError) and error.returncode == 255
    )
...
def _runtime_gpu_hydration_job_state(value: str, *, request_sha256: str) -> tuple[str, int | None]:
    ...
    if value == "success " + request_sha256 + "\n":
        return "success", None
    match = re.fullmatch(r"failure " + re.escape(request_sha256) + r" ([1-9][0-9]*)\n", value)
    if match is not None:
        return "failure", int(match.group(1))
...
def _runtime_gpu_hydration_rate_limit_snapshot(
    *, instance: Mapping[str, object], request_sha256: str, runner: Runner,
) -> dict[str, object] | None:
    """Ask the lease for classified evidence only; never return its stderr or token."""
    try:
        output = runner((
            *_runtime_hydration_ssh_prefix(instance),
            _runtime_gpu_hydration_rate_limit_terminal_command(request_sha256),
        ))
    except BaseException as error:
        if _runtime_hydration_transport_error(error):
            return None
        raise ValueError("runtime hydration terminal is not an authenticated exhausted Hub rate limit") from error
    return _runtime_gpu_hydration_rate_limit_terminal(output, request_sha256=request_sha256)


def _runtime_gpu_hydration_recovery_receipt(
    *, receipt: object, identity: Mapping[str, object],
) -> dict[str, object]:
    """Adopt only the complete immutable hydration/mount contract from recovery."""
    deployment = identity["deployment"]
    if not isinstance(deployment, Mapping) or not isinstance(receipt, Mapping):
        raise ValueError("runtime hydration recovery receipt is not bound to the approved deployment")
    expected = {
        "schema_version": 1, "kind": "runtime_mixture_hydration",
        "repository": deployment.get("repository"),
        "immutable_revision": deployment.get("immutable_revision"),
        "remote_prefix": deployment.get("remote_prefix"),
        "mixture_id": deployment.get("mixture_id"),
        "artifact_tree_sha256": _hash(deployment.get("artifact_entries")),
...
def validate_runtime_gpu_hydration_recovery_request(
    *, instance: Mapping[str, object], request: Mapping[str, object],
    ...
    """Authenticate same-lease recovery inputs before any cleanup-owning action."""
    if instance.get("kind") != "runtime_mixture_gpu_warmup_instance":
        raise ValueError("runtime hydration rate-limit recovery requires the direct GPU lease")
    identity = _runtime_hydration_identity(instance, request)
    request_sha256 = _runtime_gpu_hydration_request_sha256(request)
    capability_path = request.get("bootstrap_capability_receipt")
    if type(capability_path) is not str or not Path(capability_path).is_absolute():
        raise ValueError("runtime hydration rate-limit recovery requires the same-lease capability receipt")
    _runtime_gpu_bootstrap_capability_receipt(
        path=Path(capability_path), instance=instance, request=request, identity=identity,
    )
    capability_sha256 = instance.get("capability_sha256")
    if type(capability_sha256) is not str or re.fullmatch(r"[0-9a-f]{64}", capability_sha256) is None:
        raise ValueError("runtime hydration rate-limit recovery capability binding is invalid")
    return {
...
def _run_runtime_gpu_hydration_recovery_validated(
    *, instance: Mapping[str, object], request: Mapping[str, object], runner: Runner,
    validated: Mapping[str, object], sleep: Callable[[float], None] = _bounded_sleep,
) -> dict[str, object]:
    request_sha256 = validated.get("request_sha256")
    capability_sha256 = validated.get("capability_sha256")
    identity = validated.get("identity")
    if (
        type(request_sha256) is not str or re.fullmatch(r"[0-9a-f]{64}", request_sha256) is None
        or type(capability_sha256) is not str or re.fullmatch(r"[0-9a-f]{64}", capability_sha256) is None
    ...
    for poll in range(RUNTIME_HYDRATE_STATUS_POLLS):
        ...
        try:
            state, _exit_code = _runtime_gpu_hydration_job_state(status, request_sha256=request_sha256)
        except ValueError as error:
            raise ValueError("runtime hydration recovery job status is unavailable or tampered") from error
        if state == "success":
            try:
                output = runner((*ssh_prefix, _runtime_gpu_hydration_recovery_job_receipt_command(request_sha256)))
            except BaseException as error:
                if _runtime_hydration_transport_error(error):
                    return _runtime_gpu_hydration_pending(
                        instance=instance, request_sha256=request_sha256,
                    ) | {"rate_limit_recovery": True}
                raise ValueError("runtime hydration recovery receipt is unavailable or tampered") from error
            break
        if state == "failure":
            raise ValueError("runtime hydration recovery worker failed with a terminal nonzero exit")
        if state == "disappeared":
            raise ValueError("runtime hydration recovery worker disappeared without a terminal marker")
        if poll + 1 < RUNTIME_HYDRATE_STATUS_POLLS:
            sleep(RUNTIME_HYDRATE_POLL_SECONDS)
    else:
        return _runtime_gpu_hydration_pending(
    ...
    try:
        ...
        raise ValueError("runtime hydration recovery did not return an authenticated receipt") from error
    receipt = _runtime_gpu_hydration_recovery_receipt(receipt=receipt, identity=identity)
    intent_path = _runtime_gpu_hydration_recovery_intent_path(
        instance=instance, request_sha256=request_sha256,
    )
    return {
        "paid_action": True, "action": "runtime-hydrate-recover", "instance_id": instance["instance_id"],
        "rate_limit_recovery": True, "hydration_receipt": receipt,
        "recovery_intent_sha256": sha256_file(intent_path),
    }


def runtime_mixture_hydrate_recover(
    *, instance: Mapping[str, object], request: Mapping[str, object], runner: Runner,
) -> dict[str, object]:
    """Public same-lease recovery API for the single authenticated Hub-rate terminal."""
    return _run_runtime_gpu_hydration_recovery_validated(
        instance=instance, request=request, runner=runner,
        validated=validate_runtime_gpu_hydration_recovery_request(instance=instance, request=request),
    )
...
def runtime_mixture_hydrate(
    *, instance: Mapping[str, object], request: Mapping[str, object], runner: Runner,
    ...
    if instance.get("kind") == "runtime_mixture_gpu_warmup_instance":
        ...
        for poll in range(RUNTIME_HYDRATE_STATUS_POLLS):
            ...
            if state == "success":
                try:
                    ...
                    raise ValueError("runtime hydration job receipt is unavailable or tampered") from error
                break
            if state == "failure":
                try:
                    terminal = _runtime_gpu_hydration_rate_limit_snapshot(
                    ...
                    raise ValueError("runtime hydration remote job failed with a terminal nonzero exit") from error
                if terminal is None:
                    return _runtime_gpu_hydration_pending(
                        instance=instance, request_sha256=request_sha256, unresolved=True,
                    )
                if terminal is not None:
                    return {
                        "paid_action": True, "action": "runtime-hydrate",
                        "instance_id": instance["instance_id"], "pending": True,
                        "rate_limit_recovery_required": True,
                        "runtime_hydrate_request_sha256": request_sha256,
                        "partial_sources": terminal,
                    }
        ...
        command = (
            "set -eu; HF_TOKEN=\"$(cat /prepared/config/runtime.token)\" "
            "PYTHONPATH=/prepared/code/source/lehome:/prepared/code/trainer/src "
...
def remote_action(*, action: str, instance: Mapping[str, object], request: Mapping[str, object], runner: Runner) -> dict[str, object]:
    ...
    if action == "runtime-hydrate-recover":
...
def main_for_test(
    ...
    if args.action == "runtime-hydrate":
        return remote_action(
            action=args.action, instance=instance, request=request, runner=runner,
        )
    if args.action == "runtime-hydrate-recover":
        validated = validate_runtime_gpu_hydration_recovery_request(
            instance=instance, request=request,
        )
        return _runtime_abort_on_failure(
