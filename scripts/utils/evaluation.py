...
def _persistent_collection_strategy(assignment: Mapping[str, Any]) -> str:
    ...
    from lehome.flywheel.persistent_manifest import persistent_collection_strategy

    return persistent_collection_strategy(assignment)
...
def _persistent_policy_identity(args: argparse.Namespace) -> tuple[str, str, int, str]:
    ...
    from lehome.flywheel.persistent_manifest import persistent_policy_identity

    return persistent_policy_identity(args)
...
def _write_persistent_flywheel_manifest(
    ...
    *,
    ...
    return write_persistent_flywheel_manifest(
        attempt_output_dir, assignment, args, verified_restore=verified_restore,
    )
        ...
        for st in range(args.max_steps):
            ...
            if recorder is not None:
                ...
                if policy_action_diagnostics.get("policy_action_limits_available") is not True:
                    raise SimulatorNumericalDivergenceError(
                        "flywheel action validation requires available live soft joint limits"
                    )
                nonfinite_count = policy_action_diagnostics.get(
                    "policy_action_nonfinite_count"
                )
                if type(nonfinite_count) is not int or nonfinite_count != 0:
                    raise SimulatorNumericalDivergenceError(
