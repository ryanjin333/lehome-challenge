...
def _round(root: Path, attempt_id: str) -> tuple[str, str]:
    ...
    for file in sorted(path for path in attempt.rglob("*") if path.is_file()):
        checksums.append(f"{_sha_path(file)}  {file.relative_to(attempt).as_posix()}")
