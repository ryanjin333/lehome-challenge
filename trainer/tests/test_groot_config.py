...
def test_runtime_resume_cursor_is_operational_and_derives_from_checkpoint_step() -> None:
    ...
    assert first.identity()["runtime_resume_global_step"] == 0
    assert resumed.identity()["runtime_resume_global_step"] == 1
...
def test_runtime_resume_cursor_must_match_the_bound_checkpoint_step() -> None:
    with pytest.raises(ValueError, match="runtime resume offset"):
        config(
