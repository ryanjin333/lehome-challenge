...
def test_rollout_image_is_pinned_headless_and_secret_free() -> None:
    ...
    assert "/opt/conda/envs/behavior/bin/python -c 'import b1k_rollout.policy_server'" in dockerfile
    assert "python3 -c 'import b1k_rollout.policy_server'" not in dockerfile
    assert "UV_HTTP_TIMEOUT=1200" in dockerfile
    assert "WORKDIR /" in dockerfile
    assert "git init /tmp/behavior-src" in dockerfile
    assert "git init /behavior-src" not in dockerfile
    assert "rm -rf /behavior-src" in dockerfile
    assert "mv /tmp/behavior-src /behavior-src" in dockerfile
...
def test_rollout_image_installs_groot_without_reusing_uv_archive_links() -> None:
    dockerfile = _read("Dockerfile")
    ...
    assert "import gr00t, torch" in dockerfile
    assert '/opt/isaac-groot/.venv/bin/python -c "import gr00t, torch"' in verifier
...
def test_image_verifier_accepts_only_immutable_rollout_digests() -> None:
    ...
    assert "--exclude-dir=site-packages" in verifier
