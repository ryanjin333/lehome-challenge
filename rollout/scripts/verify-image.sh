...
docker run --rm --platform linux/amd64 --entrypoint /bin/bash "$image_ref" -euo pipefail -c '
  ...
  /opt/isaac-groot/.venv/bin/python -c "import gr00t, torch"
  ...
  set +e
  grep -RIE "hf_[A-Za-z0-9]{30,}|github_pat_[A-Za-z0-9_]{20,}|ghp_[A-Za-z0-9]{30,}" \
    --exclude-dir=.venv --exclude-dir=site-packages --exclude-dir=dist-packages \
