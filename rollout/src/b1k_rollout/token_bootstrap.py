"""No-follow, atomic Hugging Face token-file installation for the root bootstrap."""
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="b1k-token-bootstrap")
    parser.parse_args()
    try:
        install_token(sys.stdin.read())
    except TokenBootstrapError as error:
        print(f"b1k-token-bootstrap: {error}", file=sys.stderr)
        return 64
    return 0
